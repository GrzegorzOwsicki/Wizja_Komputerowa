...
def update_mask(_):
    ...
    lower_skin1 = np.array([h_min, s_min, v_min], dtype=np.uint8)
    upper_skin1 = np.array([h_max, s_max, v_max], dtype=np.uint8)
    lower_skin2 = np.array([h2_min, s2_min, v2_min], dtype=np.uint8)
    upper_skin2 = np.array([h2_max, s2_max, v2_max], dtype=np.uint8)
    ...
    for contour in contours:
        if cv2.contourArea(contour) > 500:
            ...
            # Check if the contour shape is ellipse-like
            if len(contour) >= 5:
                ellipse = cv2.fitEllipse(contour)
                aspect_ratio = ellipse[1][0] / ellipse[1][1]  # Width/Height
                if 0.5 < aspect_ratio < 2:  # Ensure the shape is roughly oval
...
image = cv2.imread('D:/wizja/wizja/wizjatest/image2.jpg')
